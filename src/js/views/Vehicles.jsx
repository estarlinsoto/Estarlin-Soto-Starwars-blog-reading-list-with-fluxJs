            ...
            <div className="card m-2 " key={i} >
              ...
              <div className="card-body text-white text-center" key={i}>
                ...
                <Link to={e.url.length > 31 ? `/characters/people/${e.url.slice(-3, -1)}` : `/characters/people/${e.url.slice(-2, -1)}`} className="mx-4">
                  <button className="btn btn-warning " onClick={() => actions.selectedCharacter(i)}>Learn more!</button>
                </Link>
        ...
        <div className=" row ">
          <div className=" col text-center my-3" >
            <button type="button" className={store.charsPagination === 10 || store.charsData.length === 0 ? "hide" : store.charsData.length != page ? "hide" : " btn btn-warning w-50 p-2"}
              onClick={() => actions.nextPageFunc("next", setPage(page + 1))} >More Chars</button>

            <div className={store.charsPagination === 10 || store.charsData.length === 0 ? "hide" : store.charsData.length == page ? "hide" : "spinner-grow text-warning"} role="status">
            </div>
